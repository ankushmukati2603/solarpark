...
class MainController extends Controller
{
    ...
    public function application(Request $request, $id=NULL){
        if($request->isMethod('post'))
        {
            if($request->type == 'general'){
                ...
                'ceo_mail'=>'required|email',
                'ceo_contact_number'=>'required|numeric|digits:10',
                'state'=>'required',
                ...
                // 'date'=>'required',
                'address'=>'required|max:255|regex:/^[a-zA-Z.,-_ ]+$/u',
                ...
                'dpr_status'=>'required|in:A,B,C,D',
                ],
                [
            ...
            if($request->type == 'internal_infrastructure'){
                $validation = Validator::make($request->all(), [
                'land_status_identified'=>'required|numeric|gte:0',
                'land_status_aquired'=>'required|numeric|gte:0',
                'govt_land_identified'=>'required|numeric|gte:0',
                'govt_land_acquired'=>'required|numeric|gte:0',
                'private_land_acquired'=>'required|numeric|gte:0',
                'private_land_identified'=>'required|numeric|gte:0',
                'others'=>'required',
                'road_status'=>'required|in:A,B,C',
                'park_boundary'=>'required|numeric|gte:0',
                'road_distance'=>'required|numeric|gte:0',
            ...
            if($request->type == 'internal_transmission_system'){
                ...
                'connection_point'=>'required|in:A,B',
                'whether_applied'=>'required|in:A,B',
                'external_details'=>'required|regex:/^[a-zA-Z.,-_ ]+$/u',
                'external_status'=>'required|regex:/^[a-zA-Z.,-_ ]+$/u',
                'ext_responsibility'=>'required|in:A,B',
            ...
            if($request->type == 'solar_projects'){
                ...
                'nit_date'=>'required|after:yesterday',
                'bidders_name'=>'required|regex:/^[a-zA-Z ]+$/u',
                'TD_capacity'=>'required|min:0',
                'tariff'=>'required|min:0',
                'spds_name_loa'=>'required|regex:/^[a-zA-Z ]+$/u',
                'capacity_loa'=>'required|min:0',
                'psa_date'=>'required|after:yesterday',
                'discom_name'=>'required|regex:/^[a-zA-Z ]+$/u',
                // 'psa_signature'=>'required',
                'spds_name_ppa'=>'required|regex:/^[a-zA-Z ]+$/u',
                'ppa_capacity'=>'required|min:0',
                'ppa_date'=>'required|after:yesterday',
                'spds_name_scod'=>'required|regex:/^[a-zA-Z ]+$/u',
                'scod_capacity'=>'required|min:0',
                'scod_date'=>'required|after:yesterday',
                'extended_spds_name'=>'required|regex:/^[a-zA-Z ]+$/u',
                'extended_capacity'=>'required|min:0',
                'extended_date'=>'required|after:yesterday',
            ...
            if($request->type == 'award_of_work'){
                ...
                'whether_awarded'=>'required|in:A,B',
                ...
                try {
                    ...
                    if($request->editId){
                        ...
                        $auditData = array('action_type'=>'3','description'=>'User Update Award Of Work Data','user_type'=>'0');
                        $this->auditTrail($auditData);
                        return response()->json(['status' => 'success','message'=>'Details save successfuly!','next'=>$request->next,'url'=>'no','redirect'=>'yes']);
                    }  
                } catch (\Throwable $th) {
            ...
            if($request->type == 'solar_park_completion'){
                ...
                try {
                    ...
                    if($request->hasFile('upload_file')){
                        $rd=rand(6,778798);
                        $file= $this->uploadFile($request->file('upload_file'), $dir_path,'UPLOADFILE'.$rd.'_');
                        //$uploadedfile=$file['name'];
                        $solarparkCompletion['upload_file']=$file['name'];
                    }
                    if($request->editId){
            ...
            if($request->type == 'status_of_release_of_cfa'){
                $validation = Validator::make($request->all(), [
                'land_acquisition_amount'=>'required|numeric|gte:0',
                'financial_closure_amount'=>'required|numeric|gte:0',
                'aw_pooling_station_amount'=>'required|numeric|gte:0',
                'receipt_material_site_amount'=>'required|numeric|gte:0',
                'completion_construction_amount'=>'required|numeric|gte:0',
                'land_acquisition_due_date'=>'required|date',
                'financial_closure_due_date'=>'required|date',
                'aw_pooling_station_due_date'=>'required|date',
                'receipt_material_site_due_date'=>'required|date',
                'completion_construction_due_date'=>'required|date',
                'land_acquisition_release_date'=>'required|date',
                'financial_closure_release_date'=>'required|date',
                'aw_pooling_station_release_date'=>'required|date',
                'receipt_material_site_release_date'=>'required|date',
                'completion_construction_release_date'=>'required|date',  
                ]);
                if ($validation->fails()){   //check all validations are fine, if not then redirect and show error messages
                    return response()->json(['status'=>'verror','data'=>$validation->errors()]);
                }
                try {
                    ...
                    if($request->editId){
                        ...
                        $auditData = array('action_type'=>'3','description'=>'User Update Status of Release of CFA Data','user_type'=>'0');
                        $this->auditTrail($auditData);
                        return response()->json(['status' => 'success','message'=>'Details save successfuly!','next'=>$request->next,'url'=>'no','redirect'=>'yes']);
                    }
                } catch (\Throwable $th) {
            ...
            if($request->type == 'attachments'){
                ...
                $validation = Validator::make($request->all(), [
                    'site_photo'=>'required',
                    // 'site_photo.*' => 'mimetypes:image/jpeg,image/png',
                    
                    'road_photo'=>'required',
                    // 'road_photo.*'=>'mimetypes:image/jpeg,image/png',
                    // 'road_photo.*' => 'mimes:jpg,png,jpeg|max:10024',.
                    'ipes_photo'=>'required',
                    // 'ipes_photo.*'=>'required|mimetypes:image/jpeg,image/png',
                    // 'ipes_photo.*' => 'mimes:jpg,png,jpeg|max:10024',
                    'exts_photo'=>'required',
                    // 'exts_photo.*'=>'required|mimetypes:image/jpeg,image/png',
                    // 'exts_photo.*' => 'mimes:jpg,png,jpeg|max:10024',
                    ...
                    'road_photo.required'=>'This field is required',
                    'road_photo.*.mimetypes'=>'Please select valid files',
                    'ipes_photo.required'=>'This field is required',
                    'ipes_photo.*.mimetypes'=>'Please select valid files',
                    'exts_photo.required'=>'This field is required',
            ...
            if($request->type == 'additional_information'){
                ...
                $validation = Validator::make($request->all(), [
                    'other_documents'=>'required|mimes:doc,docx,pdf',
                ],
                [
            ...
            if($request->type == 'final_submission'){
                ...
                $validation = Validator::make($request->all(), [
                    ...
                    'authorize' =>  'required',
                    'project_report_detail' =>  'required',
                    'expenditure_statement' =>  'required',
                ],
                [
                ...
                try {
                    ...
                    if($request->hasFile('project_report_detail')){
                        $rd=rand(6,778798);
                        $file= $this->uploadFile($request->file('project_report_detail'), $dir_path,'PROJECT_REPORT_'.$rd.'_');
                        $project_report_detail=$file['name'];
                    }
                    if($request->hasFile('expenditure_statement')){
                        $rd=rand(6,778798);
                        $file= $this->uploadFile($request->file('expenditure_statement'), $dir_path,'EXPENDITURE_STATEMENT_'.$rd.'_');
                        $expenditure_statement=$file['name'];
                    }
                    if($request->editId){
                        $url = urlencode('/'.Auth::getDefaultDriver().'/my-progress-report');
                        ProgressReport::where('id',$request->editId)->update(
                            [
                                'project_report_detail' => $project_report_detail,
                                'expenditure_statement' => $expenditure_statement,
        ...
        if($data!=null){
            ...
            $generalData['project_report_detail'] = $data->project_report_detail;
            $generalData['expenditure_statement'] = $data->expenditure_statement;
            $generalData['undertaking'] = $data->undertaking;
            $generalData['solar_park_completion'] = json_decode($data->solarpark_completion,true);
            //dd($generalData);
            $states = State::All();
    ...
    public function newProgressReport(Request $request){
        if($request->isMethod('post')){
            ...
            $month=$request->month;
            $year=$request->year;
            $existingProgressData=ProgressReport::select('*')->where('user_id', Auth::id())->where('month', $month)->where('year',$year)->where('park_id',$request->solar_park_name)->first();
            // dd($existingProgressData);
            if($existingProgressData!=null){
                if($existingProgressData->final_submission==0){
                    $auditData = array('action_type'=>'1','description'=>'visit Progress Report (General)Page','user_type'=>'0');
                    $this->auditTrail($auditData);
                    $url=urlencode('/'.Auth::getDefaultDriver().'/application/progress_report/'.$existingProgressData->id);
            ...
            $newProgressData->park_id=$request->solar_park_name;
        ...
        $this->auditTrail($auditData);
        return view('backend.beneficiary.progress_report.newProgressReport',compact('solarPark_name'));
    }
    public function myProgressReport(Request $request){
        
        if($request->isMethod('post')){
            // $validatedData=$request->validate([
            //     'date'=>'required_without:park_name',
            //     'park_name'=>'required_without:date',
            // ],[  
            //     'date.required_without' => 'Please provide atleast date or park name.', 
            ...
            //dd(Auth::id());
            DB::enableQueryLog();
            $query= ProgressReport::where('user_id',Auth::id());
            if($fromdata!=''){
            ...
            foreach($progressData as $data){$i++;
                ...
                if($decodeData!=null){
                    ...
                    $progressDetails[$i]['state_name'] ='NA';
                    $progressDetails[$i]['district_name'] ='NA';
                }
            }
            try {
                //code...
        ...
        $this->auditTrail($auditData);
        $progressDetails=array();
        $progressData= ProgressReport::where('user_id',Auth::id())->orderBy("submitted_on",'DESC')->get()->toArray();;
        $i=0;
        foreach($progressData as $data){$i++;
            $decodeData=json_decode($data['general'],true);
    ...
    public function previewProgressReport($id){
        ...
        //$previewData['project_report_detail'] = $progressDetailspreview->project_report_detail;
        ...
        try {
            ...
            if($request->isMethod('post')){
                ...
                if (!(Hash::check($request->current_password, Auth::user()->password))) {
                    ...
                    return response()->json(['status' => 'error','message'=>'Your current password does not matches with the password you provided. Please try again.','url'=>'']); 
                ...
                if(!$this->passwordPolicyTest($request->new_password)){
                    ...
                    $this->auditTrail($auditData);
                    /*************************Audit Trail Start**********************************/
                    return response()->json(['status' => 'error','message'=>'Your current password does not matches with the password Policy','url'=>'']); 
                ...
                if(strcmp($request->current_password, $request->new_password) == 0){
                    ...
                    return response()->json(['status' => 'error','message'=>'New Password cannot be same as your current password. Please choose a different password','url'=>'']); 
    ...
    public function editProfile(Request $request) {
        ...
        if ($request->isMethod('post')) {
            ...
            $auditMsg = "Developer update profile successfuly";
        ...
        $states = State::orderby("name")->get();
        return view('backend.beneficiary.editProfile', compact('user', 'states','url'));
    ...
    public function consolidateReport(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($status==1){
                $progressData= ProgressReport::whereBetween('submitted_on', [$fromdata.' 00:00:00' , $todata.' 23:59:59'])->where('user_id',Auth::id())->orderBy("id",'DESC')->limit(200)->get()->toArray();
            ...
            foreach($progressData as $data){$i++;
                ...
                if($decodeData!=null){
                    ...
                    $progressDetails[$i]['state_name'] ='NA';
                    $progressDetails[$i]['district_name'] ='NA';
                    $progressDetails[$i]['sub_district_name'] ='NA';
                }
    ...
    public function previewconsolidateReport($id){
        ...
        $previewData['status_of_release_of_cfa']=json_decode($consolidateDetailspreview->status_of_release_of_cfa,true);
    ...
    public function checkedPreviousReport($type,$month,$year,$id){
        ...
        if($previousprogressReport==null){
            return response()->json(['status' => 'error','message'=>'Previous Month Data not Found']);
        }else{
    ...
    public function addSolarPark(Request $request ,$id=NULL){
        ...
        if($request->isMethod('post')){
            $validation = Validator::make($request->all(), [
                'park_name'=>'required|regex:/^[a-zA-Z ]+$/u',
                'developer_name'=>'required|regex:/^[a-zA-Z ]+$/u',
                'pan_no'=>'required|regex:/^[A-Z]{5}[0-9]{4}[A-Z]{1}+$/u',
            ...
            if($request->editId){
                ...
                $auditData = array('action_type'=>'3','description'=>'User Update Solar Park data','user_type'=>'0');
                $this->auditTrail($auditData);
            }else{
                $newSPData=new ManageSolarPark();
                $auditData = array('action_type'=>'2','description'=>'User Insert Solar Park data','user_type'=>'0');
                $this->auditTrail($auditData);
            }
            ...
            return response()->json(['status' => 'success','message'=>'Details saved successfuly!','url'=>$url,'redirect'=>'yes']);
        }
        $editedSPData=array();
        if($id){
