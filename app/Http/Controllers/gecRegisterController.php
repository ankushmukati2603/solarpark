...
class gecRegisterController extends Controller
{
    use General;
public function userRegistration(Request $request)
    ...
    public function loginOtp(Request $request){
         $phone=$request->number;
        $data=Gecd::where('contact_no',$phone)->first();
        if($data){
            Otp::where('otp_title', $phone)->delete();
            // $otpPhone=$this->generateOtp();
            $otpPhone= 123456 ;
    ...
    public function userOTPVerified(Request $request){
        ...
        if($data){
            ...
            return response()->json(['status'=>'success','massage'=>'OTP Verification successfully']);
        }
        return response()->json(['status'=>'error','massage'=>'OTP  verification failed']); 
    }
    
    public function gecLogIn(){

        return view('gecLogIn');
    }
