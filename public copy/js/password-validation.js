		...
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			function buildPasswordRegex() {
				...
				if (_options.noUpper)
					...
					cases.push({
						regex:
							...
							')',
						message: '' + _options.minUpperCase + ' upper case letters.'
					});
				if (_options.noLower)
					cases.push({
					...
					cases.push({
						regex:
							...
							')',
						message: '' + _options.minLowerCase + ' lower case letters.'
					});
				if (_options.noDigit)
					...
					cases.push({
						regex:
							...
							')',
						message: '' + _options.minDigits + ' digits.'
					});
				if (_options.noSpecial)
					...
					cases.push({
						regex:
							'(?=' +
							('[' + CHARSETS.specialSet + '][^' + CHARSETS.specialSet + ']*').repeat(
								_options.minSpecial
							) +
							')',
						message: '' + _options.minSpecial + ' special characters.'
					});

				cases.push({
					regex: '[' + charsetToString() + ']{' + _options.minLength + ',}',
					message: '' + _options.minLength + ' characters.'
				});
			...
			function validateField() {
				...
				if (
					...
					failedCases.push(
						'Password can not contain ' + _options.maxRepeats + ' of the same character case insensitive.'
					);
				}
				if (
					_options.failConsecutive &&
					$(_element).val().search(new RegExp('(?=(.)' + '\\1'.repeat(_options.maxConsecutive) + ')', 'g')) !=
						-1
				) {
					failedCases.push(
