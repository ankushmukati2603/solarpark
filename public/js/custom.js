...
function getDistrictByState(state_id, district_id, id = '') {
    var stateID = state_id;
    $('#sub_district_id' + id).html("<option value=''>Select Sub District</option>");
    $('#block_id' + id).html("<option value=''>Select Block</option>");
    $('#village_id' + id).html("<option value=''>Select Village</option>");
    $('#panchayat_id' + id).html("<option value=''>Select Panchayat</option>");
    $('#ward_id' + id).html("<option value=''>Select Ward</option>");
    var districtData = '<option value="">Select District</option>';
    if (stateID) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#district_id' + id).html(districtData);
            }
        });
    }
}
function getSubDistrictByDistrict(district_id, sub_district_id, id = '') {
    ...
    if (districtID) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#sub_district_id' + id).html(subDistrictData);
            }
        });
    }
}
...
//     }
// }
function getVillageBySubDistrict(sub_district_id, village_id, id = '') {
    ...
    if (subDistrictID) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#village_id' + id).html(villageData);
            }
        });
    }
}
...
function getLastMonthReportData(type, month, year, id) {
    ...
    if (confirm('Do you want to copy previous month report?')) {
        if (type) {
            $.ajax({
                type: 'GET',
                url: baseUrl + '/developer/checked-previous-report/' + type + '/' + month + '/' + year + '/' + id,
                //data: 'state_id=' + stateID,
                success: function (data) {
                    //alert(data.status);
...
function getLastMonthSNAReportDetails(type, month, year, id) {
    // alert(type + '-' + month + '-' + year);
    if (confirm('Do you want to copy previous month report?')) {
        if (type) {
            $.ajax({
                type: 'GET',
                url: baseUrl + '/state-implementing-agency/checked-previous-report/' + type + '/' + month + '/' + year + '/' + id,
                //data: 'state_id=' + stateID,
                success: function (data) {
                    //alert(data.status);
                    if (data.status == 'success') {
